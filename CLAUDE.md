...
| Radio | `src/COMMS/RFD900` | 900MHz serial protocol; command RX + telemetry TX on FreeRTOS task; runtime tuning via slash-delimited text frames (`O/` outer angle loop PIDs, `I/` or untagged inner rate loop PIDs, `F/` front motor bias, `R/` right motor bias) |
