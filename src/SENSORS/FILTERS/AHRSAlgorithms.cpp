							   ...
							   float deltat)
{
	...
	const float twoKp = 2.0f * Kp;
	const float twoKi = 2.0f * Ki;
	...
	if(twoKi > 0.0f)
	{
		integralFBx += twoKi * ex * deltat;
		integralFBy += twoKi * ey * deltat;
		integralFBz += twoKi * ez * deltat;
	...
	// normalize quaternion
	recipNorm = sqrt(q1*q1 + q2*q2 + q3*q3 + q4*q4);
	recipNorm = 1.0f / recipNorm;
			   ...
			   float dt)
{
	...
	float pitchAcc = atan2(-ax, sqrt(ax*ax + az*az));
	...
	a.roll  = alpha * a.roll  + (1.0f - alpha) * rollAcc;
	a.pitch = alpha * a.pitch + (1.0f - alpha) * pitchAcc;
	return a;
}
...
Attitude getAttitude() {
	...
	attitude.roll = atan2(2.0f * (w * x + y * z), 1.0f - 2.0f * (x * x + y * y));

	// Pitch (y-axis rotation)
	// asin(2*(w*y - z*x))
	attitude.pitch = asin(2.0f * (w * y - z * x));

	// Yaw (z-axis rotation)
	// atan2(2*(w*z + x*y), 1 - 2*(y*y + z*z))
	attitude.yaw = atan2(2.0f * (w * z + x * y), 1.0f - 2.0f * (y * y + z * z));

	// Convert radians to degrees
	attitude.roll *= 180.0f / PI;
	attitude.pitch *= 180.0f / PI;
	attitude.yaw *= 180.0f / PI;
	return attitude;
}
